...
find_ranges = function(cal_age, prob, range_prob, print_ranges = FALSE) {
  perm = order(prob, decreasing = TRUE)
  cum_prob = 0
  num_values = 0
  for (i in perm) {
    cum_prob <- cum_prob + prob[i]
    num_values <- num_values + 1
    if (abs(cum_prob - range_prob) < 1e-4 || cum_prob > range_prob) {
      break
    }
  }
  included_values = sort(perm[1:num_values])
  ranges = list()
  ind = 1
  start_range = cal_age[included_values[1]]
  areas = c(prob[included_values[1]])
  for (i in 2:length(included_values)) {
    if (included_values[i] - included_values[i-1] > 1) {
      ranges[[ind]] <- c(start_range, cal_age[included_values[i-1]])
      start_range = cal_age[included_values[i]]
      ind <- ind + 1
      areas[ind] <- prob[included_values[i]]
    } else {
      areas[ind] <- areas[ind] + prob[included_values[i]]
    }
  }
  ranges[[ind]] = c(start_range, cal_age[included_values[num_values]])
  if (print_ranges) {
    print(paste(range_prob*100, "% probability"))
    for (i in 1:length(ranges)) {
      print(paste("   ", round(ranges[[i]][1]), "AD (", round(areas[i] * 100, 1) ,"%) ", round(ranges[[i]][2]),"AD", sep=""))
    }
  }
  return(ranges)
}
...
## compare range calculated with our datasets against the data from OxCal
plot(
  oxcal_age,
  oxcal_prob,
  col = "blue",
...
carbondate_age = seq(646, by = 1, length.out = length(carbondate_prob))
...
plot_range(find_ranges(carbondate_age, carbondate_prob, 0.997, TRUE), "purple", 10e-4)
